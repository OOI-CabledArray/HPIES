#!/usr/bin/env python
# termsock.py -- terminal emulator to raw socket

"""
run socat to connect port 9999 with ttyR0:
  socat TCP4-LISTEN:9999,reuseaddr,fork /dev/ttyR0,raw,echo=0,b38400

connect null modem between ttyRP0 and ttyRP7

first kermit:
  kermit
  set telnet wait-for-negotiations off
  set host localhost 9999 /NO-TELNET-INIT
...
Notice that bytes go both ways.
Also notice that CR sent from first kermit is received as CR/LF on second.
But CR sent from second kermit is received as just CR on first kermit.
...
class GuiPart:
  def __init__(self, master, queue, my_quit):
    ...
    B1.grid(row=1,column=2,columnspan=2,sticky=Tkinter.W)

    B2 = Tkinter.Button(master, text='HEF Opto On', command=hef_opto_on)
    B2.grid(row=2,column=2,columnspan=4,sticky=Tkinter.W)

    B3 = Tkinter.Button(master, text='HEF Power On', command=hef_pwr_on)
    B3.grid(row=3,column=2,sticky=Tkinter.W)

    B4 = Tkinter.Button(master, text='HEF Wake', command=hef_wake)
    B4.grid(row=4,column=2,columnspan=4,sticky=Tkinter.W)

    C1 = Tkinter.Button(master, text='STM DAQ Stop', command=stm_daq_stop)
    C1.grid(row=1,column=3,columnspan=2,sticky=Tkinter.W)
    ...
    B7.grid(row=7,column=2,sticky=Tkinter.W)
    ...
    B6.grid(row=6,column=2,sticky=Tkinter.W)
    ...
    B8.grid(row=8,column=2,sticky=Tkinter.W)
    ...
    B14.grid(row=14,column=2,columnspan=2,sticky=Tkinter.W)

    B14 = Tkinter.Button(master, text='Plot Cal E2', command=plot_cal_e2)
    B14.grid(row=14,column=3,columnspan=2,sticky=Tkinter.W)

    B20 = Tkinter.Button(master, text='Show HEF Params', command=hef_params)
    B20.grid(row=20,column=2,columnspan=4,sticky=Tkinter.W)
    ...
    B22.grid(row=22,column=2,columnspan=4,sticky=Tkinter.W)

    B23 = Tkinter.Button(master, text='HEF Stats', command=hef_stats)
    B23.grid(row=23,column=2,columnspan=4,sticky=Tkinter.W)

    C22 = Tkinter.Button(master, text='HEF Stop', command=hef_mission_stop)
    C22.grid(row=22,column=3,columnspan=4,sticky=Tkinter.W)
    ...
    B24e = Tkinter.Entry(master, width=15)
    B24e.delete(0, Tkinter.END)
    if options.jbox_hostname:
      lbl = options.jbox_hostname
    else:
      lbl = str.format('{0}',stm_baud)
    B24e.insert(0, lbl)
    B24e.grid(row=24,column=3, columnspan=1,sticky=Tkinter.W)
    B24e.focus_set()
    def B24_call_back():
      baud_str = B24e.get()
      new_baud(baud_str)
    if options.jbox_hostname:
      ...
      lbl = 'New Baud Rate:'
    B24 = Tkinter.Button(master, text=lbl, command=B24_call_back)
    B24.grid(row=24,column=2, columnspan=1,sticky=Tkinter.E)
