...
def check_crc_aux(line):
  ...
  print('ERROR: check_crc_aux: i=',i,'j=',j)
  print('line=',line)
  sys.exit(0)
  return True
...
def check_crc(line):
  ...
  try:
    ...
    crc_rcv = None
    print('WARNING: bad crc decode, lineno=',lineno,'linein=',repr(linein))
    count_bad_crc_decode += 1
    return False

  if crc_rcv != None:
    if crc_computed != crc_rcv:
      print('WARNING: crc error, lineno=',lineno,'linein=',repr(linein))
...
#   print_info_and_exit()
  ...
  if chan==5 and len(aux_split) == 13:
    try:
      ...
      print('WARNING: cannot decode aux split 13',line)
...
#4_AUX,1437846000,04,390234,390368,390423,390435,2954780,001396,001410,04000018.086,3745\r\r\n*e0f0
    ...
    print('WARNING: AUX wrong split len:',len(aux_split))
    print('  lineno=',lineno)
    print('  line=',line)
...
def decode_hef_hdr():
  ...
  if len(hef_split) < 1:
    print('ERROR: len(hef_split)=',len(hef_split))
    print_info_and_exit()

  # check if this is a header for EF, cal or motor
  if hef_split[0].find('#3__HE') == 0:
    if HEF_HDR.hef_iscan != -1:
      print('WARNING: decode_hef_hdr(): hef_iscan=',HEF_HDR.hef_iscan)
      return
      # print_info_and_exit()

    global hef_ntoss
    if hef_ntoss > 0:
      print('WARNING: previous hef_ntoss=',hef_ntoss)
    hef_ntoss = 0
    ...
    if   hef_split[0] == '#3__HE04':
      ...
      print('ERROR: unknown he_hdr_ver')
      print_info_and_exit()

    if he_hdr_ver == 4 or he_hdr_ver == 5:
      if len(hef_split) != 12:
        print('ERROR: HE05 wrong split len:',len(hef_split))
...
#       HEF_HDR.uxt      = int(hef_split[6])
        ...
        HEF_HDR.uxt      = int(hef_split[11])
      except:
        print('ERROR: error decoding HE05')
        print_info_and_exit()
      ...
      if HEF_HDR.uxt < 1372636800:                    # July 1, 2013
        ...
        print('ERROR: not sure how to compute uxt_ibeg between OKMC and RSN')
        print_info_and_exit()
      ...
      if HEF_HDR.typ == 'f' or HEF_HDR.typ == 'r':
        ...
        count_hef_unknown += 1
        print('ERROR: unknown HEF hdr type')
        print_info_and_exit()

      if HEF_HDR.abu != 'a' and HEF_HDR.abu != 'b' and HEF_HDR.abu != 'u':
        print('ERROR: HEF_HDR.abu should be a, b or u')
        print_info_and_exit()

      if HEF_HDR.navg_mot < 1 or HEF_HDR.navg_ef < 1 or HEF_HDR.navg_cal < 1:
        print('ERROR: HEF_HDR: navg_mot, navg_ef, navg_cal must all be > 0')
        print_info_and_exit()
...
def decode_cal_status():
  ...
  if hef_split[0].find('#3__SC') == 0:
    ...
    if len(hef_split) != 4:
      print('WARNING: cal status header record should have 4 tokens')
  ...
  if hef_split[0].find('#3__Sc') == 0:
    if len(hef_split) != 3:
      print('WARNING: cal status data record should have 3 tokens')
    ...
    if ok:
      ...
      if CAL_STATUS.lno == CAL_STATUS.nl:
        if options.verbose:
          ...
          sys.stdout.flush()
        if expand_cal_status() == False:
          return
        if options.verbose:
          print('finished expand_cal_status()')
          sys.stdout.flush()
        if compute_cal_demod() == False:
          return
        if options.verbose:
...
def expand_cal_status():
  ...
  if len(CAL_STATUS.stat_fast) != CAL_STATUS.ns:
    print('WARNING: CAL_STATUS.stat_fast has wrong length')
    return False
    print_info_and_exit()
  ...
  if options.verbose:
    print('np.min(HCYC.ind)=',np.min(HCYC.ind))
    print('np.max(HCYC.ind)=',np.max(HCYC.ind))
    print('len(CAL_STATUS.stat_fast)=',len(CAL_STATUS.stat_fast))

  if options.verbose:
    print('type(HCYC.ind)=',type(HCYC.ind))
  HCYC.inda = np.array(HCYC.ind)
  if options.verbose:
    print('type(HCYC.inda)=',type(HCYC.inda))
    print('type(CAL_STATUS.stat_fast)=',type(CAL_STATUS.stat_fast))

  j = np.nonzero(HCYC.inda < len(CAL_STATUS.stat_fast))[0]
  if options.verbose:
    print('type(j)=',type(j))
    print('len(j)=',len(j))
    print('np.max(j)=',np.max(j))
  if np.max(j) >= len(HCYC.inda):
    print('some HCYC.inda exceed len of CAL_STATUS.stat_fast')
  try:
    CAL_STATUS.hcyc_status = CAL_STATUS.stat_fast[HCYC.inda[j]]
  except:
    print('WARNING: cannot select all CAL_STATUS.hcyc_status')
#   print('  HCYC.ind=',HCYC.ind)
    print('  HCYC.inda[j]: max=',np.max(HCYC.inda[j]),', min=',np.min(HCYC.inda[j]))
    print('len(CAL_STATUS.stat_fast)=',len(CAL_STATUS.stat_fast))
    print('CAL_STATUS.ns=',CAL_STATUS.ns)
    j = np.nonzero(HCYC.inda < len(CAL_STATUS.stat_fast))[0]
    CAL_STATUS.hcyc_status = CAL_STATUS.stat_fast[HCYC.inda[j]]
    return False
    print_info_and_exit()
  ...
  if len(CAL_STATUS.neg) + len(CAL_STATUS.pos) != len(CAL_STATUS.hcyc_status):
    print('WARNING: some unknown values in CAL_STATUS.hcyc_status')
    return False
    print_info_and_exit()
...
# global hef_iscan
  ...
  if hef_split[0].find('#3__D')==0 and HEF_HDR.hef_iscan < 0:
    ...
    hef_ntoss += 1
    if hef_ntoss == 1:
      print('WARNING: append_hef_data(): tossed data record without header record')
      print('  lineno=',lineno)
      print('  linein=',repr(linein))
    return
  ...
  if hef_split[0] == '#3__DM':
    ...
    try:
      ...
      print('ERROR: cannot decode motor current data')
      print_info_and_exit()
    ...
    if HEF_HDR.abu == 'a':
      ...
      print('ERROR: unknown abu=',HEF_HDR.abu)
      print_info_and_exit()
  ...
  # EF data
    ...
    if he_hdr_ver == 4:
      try:
        ...
        print('ERROR: EF data version 4 decode failed, lineno=',lineno,'linein=',repr(linein))
        print('hef_split=',hef_split)
        print_info_and_exit()
    elif he_hdr_ver == 5:
      try:
        ...
        print('ERROR: EF data version 5 decode failed, lineno=',lineno,'linein=',repr(linein))
        print('hef_split=',hef_split)
        print_info_and_exit()
    ...
    if ind != (HEF_HDR.hef_iscan + 1) * HEF_HDR.navg_ef - 1:
      print('WARNING: hprpro.py: ind != (HEF_HDR.hef_iscan + 1) * HEF_HDR.navg_ef - 1')
      print('  ind=',ind)
    ...
    if HEF_HDR.abu == 'a':
      ...
      print('ERROR: hprpro.py: unknown abu=',HEF_HDR.abu)
      print_info_and_exit()
    ...
    try:
      ...
      print('ERROR: hprpro.py: cannot append HCYE:')
      print('  hef_iscan=',HEF_HDR.hef_iscan)
      print('  lineno=',lineno)
      print('  linein=',repr(linein))
      sys.exit(1)
...
#     print('hprpro.py: hef_iscan=',HEF_HDR.hef_iscan,'hef_nscan=',HEF_HDR.hef_nscan)
    ...
    if HEF_HDR.hef_iscan > HEF_HDR.hef_nscan:
      print('ERROR: hprpro.py: hef_iscan > hef_nscan')
      print_info_and_exit()
...
#     print('#3__DC: hef_iscan=',HEF_HDR.hef_iscan)
    try:
      ...
      print('ERROR: hprpro.py: cannot decode cal data')
      print_info_and_exit()
...
#     demod_cal_flag = True  # defer demod until receive cal status
    ...
    if HEF_HDR.hef_iscan > HEF_HDR.hef_nscan:
      print('ERROR: hprpro.py: HEF_HDR.hef_iscan > hef_nscan')
      print_info_and_exit()
...
def compute_cal_demod():
  ...
  n = len(HCYC.uxt)
  if n <= 0:
    print('WARNING: hprpro.py: len demod cal arrays == 0')
    return

  if len(CAL_STATUS.hcyc_status) != n:
    print('WARNING: hprpro.py: demod_cal() len error')
    print('  len(CAL_STATUS.hcyc_status)=',len(CAL_STATUS.hcyc_status))
    print('  n=',n)
    return False
    print_info_and_exit()
  ...
  if len(np.nonzero(squarewave == 0)[0]) > 0:
    print('WARNING: hprpro.py: squarewave should have no zero value elements')
    return False
    print_info_and_exit()
...
def accum_mot_cur_dur():
  ...
  if MOT_STATUS.stat_fast is None:
    print('WARNING: hprpro.py: accum_mot_cur_dur: MOT_STATUS.stat_fast == None')
    return

  if MOT_STATUS.stat_pick is None:
    print('WARNING: hprpro.py: accum_mot_cur_dur: MOT_STATUS.stat_pick == None')
    return
  ...
  if len(MOT_STATUS.stat_pick) != len(HCYM.uxt):
    print('WARNING: hprpro.py: accum_mot_cur_dur: lengths not right')
    return
  ...
  if len(jp1)>0 and len(jp2)>0:
    ...
    MOT_CUR.cur1_max.append(np.max(HCYM.curnp[jp1]))
    MOT_CUR.cur2_max.append(np.max(HCYM.curnp[jp2]))
    MOT_CUR.cur1_med.append(np.median(HCYM.curnp[jp1]))
    MOT_CUR.cur2_med.append(np.median(HCYM.curnp[jp2]))
...
def compute_mot_cur_dur():
  ...
  MC.cur1_max  = np.array(MOT_CUR.cur1_max,dtype='double')
  MC.cur2_max  = np.array(MOT_CUR.cur2_max,dtype='double')
  MC.cur1_med  = np.array(MOT_CUR.cur1_med,dtype='double')
  MC.cur2_med  = np.array(MOT_CUR.cur2_med,dtype='double')
...
# mlt1 = 719529 - 366 + MC.uxt1 / 86400.0
# mlt2 = 719529 - 366 + MC.uxt2 / 86400.0
  ...
  axlist.append(ax)
  ax.hold(True)
  ax.plot_date(mlt1[ja], MC.cur1_max[ja],'r.')
  ax.plot_date(mlt1[jb], MC.cur1_max[jb],'b.')
  ax.plot_date(mlt1[ja], MC.cur1_med[ja],'m.')
  ax.plot_date(mlt1[jb], MC.cur1_med[jb],'c.')
  ax.hold(False)
  ...
  axlist.append(ax)
  ax.hold(True)
  ax.plot_date(mlt2[ja], MC.cur2_max[ja],'r.')
  ax.plot_date(mlt2[jb], MC.cur2_max[jb],'b.')
  ax.plot_date(mlt2[ja], MC.cur2_med[ja],'m.')
  ax.plot_date(mlt2[jb], MC.cur2_med[jb],'c.')
  ax.hold(False)
  ax.set_ylim(ylim_cur)
  ax.grid(True)
  plt.ylabel('cur2')
...
def check_hef_time():
  ...
  if hef_split[0] == '#3__HE05':
    try:
      ...
      print('WARNING: hprpro.py: bad HE05 hef_secs_diff')
      return False
      print_info_and_exit()
  return True
...
def append_compass():
  ...
  if hef_split[0].find('#3__HC') == 0:
    if hef_split[0] == '#3__HC03':
      if len(hef_split) != 8:
        print('WARNING: hprpro.py: HC03 wrong split len:',len(hef_split))
        print('  hef_split:',hef_split)
        return False
        print_info_and_exit()
      count_compass += 1
    else:
      print('ERROR: hprpro.py: unknown compass header')
      print_info_and_exit()
    ...
    try:
      ...
      print('ERROR: hprpro.py: error in decoding compass data')
      print_info_and_exit()
...
def plot_scatter():
  ...
  if n1a != n2a:
    print('ERROR: n1a & n2a should be the same')
    sys.exit(1)
  if n1b != n2b:
    print('ERROR: n1b & n2b should be the same')
    sys.exit(1)
...
#   print('uxt too short: count_raw_ef_cat=',count_raw_ef_cat)
  ...
  if count_raw_ef_cat == options.ncat:
    ...
    if len(j) != len(ja) + len(jb):
      print('ERROR: hprpro.py: plot_raw_cal_cat(): len(j) should equal len(ja) + len(jb)')
      print_info_and_exit()
...
def initcounts():
  ...
  count_lineok = 0
  count_skipped = 0
  count_skipped_GPZDA = 0
  count_removed = 0
  count_ascii_tossed = 0
  count_j1_is_zero = 0

def printcounts():
  print('hprpro.py printcounts():')
  print('  lineno              =',lineno)
  print('  count_skipped       =',count_skipped)
  print('  count_skipped_GPZDA =',count_skipped_GPZDA)
...
def print_info_and_exit():
  print('ERROR: print_info_and_exit()')
  print('  ifile=',ifile)
  print('  lineno=',lineno)
  print('  linein=',repr(linein))
  printcounts()
...
def gz_open(ifile):
  if ifile[-3:] == '.gz':
    try:
      ...
      print('ERROR: cannot open ifile=',ifile)
      sys.exit(1)
  else:
    try:
...
def plot_e12_demod(which):
  ...
  if which == 'e1':
    ...
    print('ERROR: plot_e12_demod(): unknown which=',which)
    sys.exit(1)
...
def writetxtgz(onameroot):
  ...
  try:
    ...
    print('ERROR: cannot open output file=',ofile)
    sys.exit(1)
  ...
  print('# vars  = index,hcno,uxt1,uxt2,abu,dur1,dur2,cur1_max,cur2_max,cur1_med,cur2_med',file=ofp)
  ...
  for i in range(len(MOT_CUR.uxt1)):
    ...
    print(',{0:.0f}'.format(MOT_CUR.cur1_max[i]),file=ofp,end='')
    print(',{0:.0f}'.format(MOT_CUR.cur2_max[i]),file=ofp,end='')
    print(',{0:.0f}'.format(MOT_CUR.cur1_med[i]),file=ofp,end='')
    print(',{0:.0f}'.format(MOT_CUR.cur2_med[i]),file=ofp,end='')
    print(file=ofp) # end of line
...
def mymkdir(mydir):
  try: 
    ...
    if not os.path.isdir(mydir):
      print('ERROR: cannot make directory:',mydir)
      sys.exit(1)
...
def nextlineinit(fn):
  ...
  NL.ib = 0
  NL.sv = ""
  NL.timestamp = 'U 1970-01-01 00:00:00'
  NL.uxt = 0
  NL.ndat = -1
  NL.count_j2xs = 0
  NL.count_nolf = 0
...
# print('nextline(): NL.ib=',NL.ib)
  ...
  nextra = 1024
  jb1 = NL.str[NL.ib:NL.ib+nextra].find('\xA3\x9D\x7A')
  ...
  if jb1 == 0:
    ...
    while True:
      jb1 = NL.str[NL.ib:NL.ib+nextra].find('\xA3\x9D\x7A')
      if jb1 != 0:
        print('ERROR: hprpro: NL.ib=',NL.ib,'jb1=',jb1,'NL.btwn=',repr(NL.btwn))
        exit(1)
      jb2 = NL.str[NL.ib+jb1+16:NL.ib+jb1+16+nextra].find('\xA3\x9D\x7A')
      ...
      if jb1 >= 0 and jb2 > jb1:
        ...
        jcrlf = NL.tmp.find('\r\n')
        if jcrlf >= 0:
          NL.oline = timestamp(NL.hdr) + '  ' + NL.tmp[:jcrlf+2]
          NL.tmp = NL.tmp[jcrlf+2:] # push down
          return len(NL.oline)
      else:
        print('ERROR: hprpro: NL.ib=',NL.ib,'jb1=',jb1,'jb2=',jb2)
        exit(1)
    ...
    while True:
      if options.verbose:
        print('NL.ib=',NL.ib)
      jlf = NL.str[NL.ib:NL.ib+nextra].find('\n')
      if jlf < 0:
        if len(NL.str) < NL.ib+nextra: # EOF
          print('EOF: unused bytes:',repr(NL.str[NL.ib:NL.ib+nextra]))
          return -9
        print('ERROR: NL.ib=',NL.ib,'no lf in nextra=',nextra)
        print('len(NL.str)=',len(NL.str))
        print('NL.ib+nextra=',NL.ib+nextra)
        exit(1)

      hunk = NL.str[NL.ib:NL.ib+jlf+1]
      hunk_len = len(hunk)

      if options.verbose:
        print('hunk  orig=',repr(hunk))

      jnts1 = hunk.find('<\\OOI-TS>\r\n')
      if jnts1 >= 0:
        hunk = hunk[:jnts1]
        if options.verbose:
          print('hunk jnts1=',repr(hunk))

      # check for ascii time stamp
      jts1 = hunk.find('<OOI-TS ')
      if jts1 >= 0:
        jts2 = hunk.find(' TS>\r\n')
        if jts2 < 0:
          jts2 = hunk.find(' XS>\r\n')
        if jts2 < 0:
          jts2 = hunk.find(' TN>\r\n')
        if jts2 < 0:
          print('ERROR: jts2=',jts2,'NL.ib=',NL.ib,'hunk=',repr(hunk))
          exit(1)
        ts = hunk[jts1:jts2+6]
        hunk = hunk[:jts1]
        if options.verbose:
          print('hunk  jts1=',repr(hunk))
        NL.timestamp = 'A ' + ts[8:27] # ascii time stamp
        if options.verbose:
          print('ts=',repr(ts),'NL.timestamp=',repr(NL.timestamp))

      NL.ib += hunk_len
      hunktot += hunk
      if options.verbose:
        print('hunk   tot=',repr(hunktot))

      if len(hunktot) > 0 and hunktot[-1] == '\n':
        break

    if options.verbose:
      print('hunk   fin=',repr(hunktot))

    if hunktot.find('#### at ') == 0:
      j = hunktot.find(',')
      ts = '# ' + uxt2str(int(float(hunktot[8:j])))
    else:
      ts = NL.timestamp
...
def main():
  ...
  parser.add_option("--show_aux", dest="show_aux", 
    action="store_true", default=False,
    help="print AUX2 strings with time stamps")

  parser.add_option("--show_all", dest="show_all", 
    action="store_true", default=False,
    help="print all strings with time stamps")
  ...
  if begsecs == None:
    print('ERROR: cannot decode time: begdate=',options.begdate)
    sys.exit(1)

  endsecs = decode_time(options.enddate)
  if endsecs == None:
    print('ERROR: cannot decode time: enddate=',options.enddate)
    sys.exit(1)
  ...
  for fileno in range(len(ifiles)):
    ...
    if INFO.ymd == None:
      print('ERROR: unknown ymd, iname=',iname)
      sys.exit(1)

    if INFO.runid == None:
      print('ERROR: unknown runid, iname=',iname)
      sys.exit(1)
...
#       continue
      ...
      if len(linein)<20 or linein[8] != 'T' or linein[15:17] != ' #' or linein[18] != '_':
        if options.verbose:
          print('SKP:',repr(linein))
        count_skipped += 1
        if linein.find(' GPZDA mktime() failed,') >= 0:
          count_skipped_GPZDA += 1
        continue
      elif options.show_all:
        print('ALL:',repr(linein))
      ...
      try:
        ...
        print('WARNING: bad strptime conversion, iso=',repr(iso),'linein=',repr(linein)) 
        secs = None
        continue
      if secs < begsecs:
        continue
      ...
      if options.verbose and count_lineok == 1:
        print('first linein=',repr(linein))
    ...
    if more[fileno]:
      ...
      if options.matdir:
        ...
        matdict = {'ymd':INFO.ymd, 'runid':INFO.runid,  'onameroot':onameroot, \
          ...
          'mot_abu':MC.abu, \
          'mot_uxt1':MC.uxt1, 'mot_cur1_max':MC.cur1_max, 'mot_cur1_med':MC.cur1_med, 'mot_dur1':MC.dur1, \
          'mot_uxt2':MC.uxt2, 'mot_cur2_max':MC.cur2_max, 'mot_cur2_med':MC.cur2_med, 'mot_dur2':MC.dur2, \
